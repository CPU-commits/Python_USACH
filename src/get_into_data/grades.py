...
def __enter_students_grades(groups_data, grades_data):
    students_data, selected_group = groups.get_students(groups_data)
    if len(students_data) > 0:
        ...
        key_group = groups_data[selected_group]['id']
        key_student = groups_data[selected_group]['students'][selected_student]['id']
        grades_data = __validate_keys_existence(key_group, key_student, grades_data)
        grades_data[key_group]['students_grades'][key_student].append(grade)
        json.dump(grades_data, grades_file)
        historical.historical('Insertar', 'Se insertan calificaciones de alumnos manualmente')
        grades_file.close()
    else:
    ...
    if data['success']:
        ...
        for group in data['data']:
            ...
            for student in students:
                ...
                grades_data = __validate_keys_existence(group, student, grades_data)
                for grade in grades:
                    grades_data[group]['students_grades'][student].append(float(grade))
        json.dump(grades_data, grades_file)
        grades_file.close()
        historical.historical('Insertar', 'Se insertan respuestas de alumnos con Excel')
...
def __print_instructions():
    ...
    print('2. Se va a generar un archivo pre-construido con las columnas a rellenar en la carpeta output dentro de "src"')
    print('3. Al abrirlo, habrán 3 columnas: \n A) Curso (id)\n B) Alumno (id)\n C) Calificación/Nota -> Formato "Entero.Decimal"')
    print('Si necesita saber estos valores, consultelo dentro de la opción "3) Grupos" del menú para Ingresar datos')
    print('4. Rellene tantas filas como necesite. Para posteriormente, guardar el archivo con el nombre de "grades.xlsx". Siendo este, el único nombre que puede tener el archivo')
    print('Trabaje siempre desde la hoja "Grades", de lo contrario, tampoco se podrá leer el archivo Excel')
    print('5. Colocar este archivo dentro de la carpeta "input". Si esta no está generada, puede crearla manualmente dentro de "src"')
    print('6. Finalmente, seleccionar la opción "2)" en el menú de Excel, para su lectura y posterior escritura. (Cualquier error, será notificado para su corrección)\n')
    ...
    while menu_open:
        ...
        if option == '1':
            ...
            if len(groups_data) > 0:
                ...
                print('No existen grupos a los cuales agregan notas. Cree nuevos grupos.')
        elif option == '2':
            __excel_menu()
        elif option == '3':
            menu_open = False
