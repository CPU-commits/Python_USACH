...
def __validators(value, col, row, groups_data, course=""):
    if col == 1:
        ...
        if value not in groups_data[course]:
            return (True, f"Error en B{row}: Este alumno, no existe, o no es del curso dado")
    return (False, "")
    
def __validators_group(value, col, row, groups_data, course=""):
    if course != "":
        general_validators, message = __validators(value, col, row, groups_data, course)
    else:
        general_validators, message = __validators(value, col, row, groups_data)
    if general_validators == False:
        return (False, message)
    if col == 3:
        if grades.validate_grade(str(value)) == False:
            return (True, f'Error en C{row}: La calificación no tiene formato "Entero.Decimal"')
    return (False, "")
...
def __format_data(groups_data):
    ...
    for groups in groups_data:
        key = groups['id']
        new_groups[key] = list(map(lambda student: student['id'], groups['students']))
    return new_groups
        ...
        if 'C' in dimensions[1]:
            ...
            for row in range(2, large+1):
                for col in range(1, 4):
                    value = grades_sheet[f"{get_column_letter(col)}{row}"].value
                    if col != 2:
                        error, message = __validators_group(value, col, row, format_data)
                    else:
                        error, message = __validators_group(value, col, row, format_data, grades_sheet[f"A{row}"].value)
                    if error:
                        print(message)
                        general_error = True
                    elif col == 3 and general_error == False:
            ...
            if general_error == False:
                return { 'success': True, 'data': new_grades_data }
        else:
...
def __validators_form(value, col, row, groups_data, forms_data, sheet, course=""):
    if course != "":
        general_validators, message = __validators(value, col, row, groups_data, course)
    else:
        general_validators, message = __validators(value, col, row, groups_data)
    ...
    if col == 3:
        if int(value) > len(forms_data[sheet]) - 1:
            return (False, f'Error en C{row}: Esta pregunta no se encuentra en ninguna de las preguntas almacenadas')
    elif col == 4:
        message = f'Error en D{row}: La respuesta debe ser en escala de 0 - 10 en enteros'
        if len(value) == 2:
            if value != "10":
                return (False, message)
        elif len(value) > 2:
...
def __get_sheet_key(name, forms_data):
    sheet_key = list(filter(lambda x: x['name'] == name, forms_data))
    return sheet_key    
...
def read_excel_file_forms():
    ...
    if excel_file:
        ...
        format_data_form = __format_data_forms(forms_data)
        for sheet_name in wb.sheetnames:
            sheet_key = __get_sheet_key(sheet_name, forms_data)
            if len(sheet_key) == 0:
                print(f'El nombre de formulario: {sheet_name}. No existe dentro de los formularios')
                general_error = True
                break
                ...
                for row in range(1, large+1):
                    for col in range(1, 5):
                        ...
                        if error:
                            print(message)
                            general_error = True
                        elif col == 4 and general_error == False:
                            ...
                            if group not in new_forms_data[sheet_key]:
                                new_forms_data[sheet_key][group] = [student_response]
                            else:
                                new_forms_data[sheet_key][group].append(student_response)
                if general_error == False:
                    return { 'success': True, 'data': new_forms_data }
            else:
